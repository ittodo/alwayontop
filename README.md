...
- 긴 설명은 두 줄로 표시하고, 같은 키에 여러 기능이 있으면 현재 선택 번호와 전체 개수를 표시
...
- 항목이 많은 단축키 안내는 화면에 맞게 확장하고, 넘치는 항목은 좌우 화살표와 마우스 휠로 페이지 이동
- Windows Terminal이 활성화되면 사용자 `settings.json`에 등록된 복사, 붙여넣기, 검색, 창 분할 등의 실제 키를 표시
- Visual Studio 2022·2026 연동 확장을 설치하면 현재 컨텍스트에서 실행 가능한 실제 단축키를 표시
- 전체화면 앱에서는 단축키 안내와 키보드 감지를 자동으로 멈추고, 트레이에서 지정한 앱은 창모드에서도 제외
- 트레이의 **현재 앱 단축키 안내 제외**로 프로세스별 제외를 전환하고 설정에서 목록을 관리
...
전체화면 앱 자동 제외는 기본으로 켜져 있습니다. 전체화면 게임·영상·프레젠테이션에서는 저수준 키보드 감지를 일시 해제해 입력에 간섭하지 않으며, 창 고정용 전역 단축키는 계속 동작합니다. 일반 최대화 창은 제외하지 않습니다.
